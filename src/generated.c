...
Uint32  _SHADOWING_SDL_MapRGB(SDL_PixelFormat * format, Uint8  r, Uint8  g, Uint8  b) {
  return SDL_MapRGB(format, r, g, b);
}
SDL_Window * _SHADOWING_SDL_CreateWindow(char * title, int  x, int  y, int  w, int  h, Uint32  flags) {
  return SDL_CreateWindow(title, x, y, w, h, flags);
}
int  _SHADOWING_SDL_FillRect(SDL_Surface * dst, SDL_Rect * rect, Uint32  color) {
